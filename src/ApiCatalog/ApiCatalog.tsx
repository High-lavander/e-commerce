...
export const categoryFilter = async (categoryId: string) => {
  try {
    ...
    const response = await fetch(
      `https://api.${process.env.VITE_CTP_API_REGION}.commercetools.com/${process.env.VITE_CTP_PROJECT_KEY}/product-projections/search?filter=categories.id:"${categoryId}"`,
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${accessToken}`,
